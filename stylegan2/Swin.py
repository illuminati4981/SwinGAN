...
class PatchUnEmbed(nn.Module):
    ...
    def forward(self, x, x_size):
        B, HW, C = x.shape
        x = x.transpose(1, 2).view(B, x_size[2], x_size[1], x_size[0])  # B Ph*Pw C
        return x
...
class SwinV2Decoder(nn.Module):
    ...
    def forward_features(self, x, hidden_states):
        for inx, layer_up in enumerate(self.layers_up):
            if inx == 0:
                x = layer_up(x)
            else:
                x = torch.cat([x, hidden_states[3-inx]],-1)
    ...
    def forward(self, x, hidden_states):
        x = self.forward_features(x, hidden_states)
        x = self.up_x4(x)
        return x
