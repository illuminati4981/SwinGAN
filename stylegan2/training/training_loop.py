...
from metrics.restoration_metrics import LPIPS, msssim, ssim
...
def training_loop(
	...
	while True:
		...
		if (rank == 0) and (image_snapshot_ticks is not None) and (done or cur_tick % image_snapshot_ticks == 0):
			...
			real_img_eval, deg_img_eval = copy.deepcopy(phase_real_img[0]).cpu(), copy.deepcopy(phase_deg_img[0]).cpu()
			# # FID
			# # Better closed to 0
			# fid = calculate_fid(batch_size, real_img_eval, deg_img_eval)
			# fids.append(fid)
			# print('fids: ', fids)

			# # PSNR
			# # For current implementation, when it is closed to 80, it is better.
			# psnr = calculate_psnr_pt(real_img_eval, deg_img_eval, 0, batch_size)
			# psnrs.append(psnr)
			# print('psnrs: ', psnrs)
			...
			# # Better closed to 0
			# lpips_model = LPIPS(net='alex').to('cpu')
			# lpips_value = lpips_model(real_img_eval, deg_img_eval, False, batch_size)
			# lpips.append(lpips_value)
			# print('lpips: ', lpips) 

			# SSIM
			ssim_score = ssim(real_img_eval, deg_img_eval, data_range=(-1, 1))
			ssim_scores.append(ssim_score)
			print(ssim_scores)

			# MSSSIM
			msssim_score = msssim(real_img_eval, deg_img_eval, data_range=(-1, 1))
			mssim_scores.append(msssim_score)
			print(mssim_scores)
